...
function onCreate()
	makeAnimatedLuaSprite('legs', 'characters/shaggy_god', (getProperty('boyfriend.x')), (getProperty('boyfriend.y') + 400))
	addAnimationByPrefix('legs', 'Instance', 'solo_legs', 24, false)
	addOffset('legs', 'Instance', 0, 0)	
        setProperty('legs.flipX', true)
	...
	makeLuaSprite('door', 'doorframe', (getProperty('boyfriend.x')), (getProperty('boyfriend.y') + 400))
	scaleObject('door', 0, 0)
	setProperty('door.alpha', 0)
	setObjectOrder('door', getObjectOrder('boyfriendGroup'))
	addLuaSprite('door', false)


	makeLuaSprite('frame', 'zephyr/part/frame', 230, 600)
...
function onUpdate(elapsed)
	if isStoryMode and not seenCutscene then
		if getProperty('boyfriend.animation.curAnim.name') == "snap" and getProperty('boyfriend.animation.curAnim.finished') == true then
			playAnim('boyfriend', 'snapped')
		...
		for i = 1, 3 do
			...
			setProperty('Debris'..i..'.angle', getProperty('Cdebris'..i..'.angle') - (dhsp[i] / 2))
		end
	end
	...
	if getProperty('boyfriend.animation.curAnim.name') == "smile" and getProperty('boyfriend.animation.curAnim.finished') == true and trigger == 1 then
		if bfControlY >= 400 then
			triggerEvent('startDialogue', 'dial1B', '');
		else
			doTweenX('camGY', 'camFollowPos', getMidpointX('boyfriend'), 1, 'elasticInOut')
			doTweenY('camGF', 'camFollowPos', getMidpointY('boyfriend'), 1, 'elasticInOut')
...
function onGodEnd()
	...
	elseif steps == 3 then
		...
		return Function_Stop;
	elseif steps == 4 then
		doTweenAlpha('byebye', 'boyfriend', 0, 2, 'linear')
...
function onTimerCompleted(tag)
	...
	if tag == "shaggy_up" then
		playing('boyfriend', 'idle')
		playSound('god-eater/shagFly', 1, 'startC')
		shag_fly = true
	end
	if tag == "dial1" then
		playAnim('boyfriend', 'smile')
	end
	if tag == "normal" then
		endCheck = true
		...
		setProperty('camFollowPos.x', getProperty('boyfriend.x') + 300)
		setProperty('camFollowPos.y', getProperty('boyfriend.y') + 300)
...
function onTweenCompleted(tag)
	...
	if string.find(tag,'boyfriend') ~= nil and string.find(tag,'Bye') then
        for ghosts = currentGhost,lastGhost do
            local spriteName = 'boyfriendGhost'..ghosts
